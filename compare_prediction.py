...
def load_alphafold_features(pkl_path):
...
def predict_from_fasta(fasta_path, model_path, device=None):
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    seq = read_fasta_sequence(fasta_path)
    print(f"✅ Secuencia cargada ({len(seq)} AA): {seq[:50]}...")

    input_tensor = sequence_to_tensor(seq).to(device)
    ...
    with torch.no_grad():
        preds = model(input_tensor, padding_mask)

    return {
        "aatype": preds["aatype"].argmax(-1).cpu().numpy(),
        "residue_index": preds["residue_index"].argmax(-1).cpu().numpy(),
        "msa": preds["msa"].argmax(-1).cpu().numpy(),
        "deletion_matrix": preds["deletion_matrix"].cpu().numpy(),
    }

# 🔹 Funciones para procesar y comparar features
def process_af2_features(features):
    aatype = features["aatype"]
    if aatype.ndim == 2 and aatype.shape[1] == 21:
        aatype = aatype.argmax(axis=-1)

    residue_index = features["residue_index"]

    msa = features["msa"]
    if msa.ndim == 3 and msa.shape[2] == 21:
        msa_consensus = msa.mean(axis=0).argmax(axis=-1)
    elif msa.ndim == 2 and msa.shape[1] == 21:
        msa_consensus = msa.argmax(axis=-1)
    else:
        msa_consensus = msa[0] if msa.ndim == 2 else msa

    deletion = features["deletion_matrix_int"]
    deletion_mean = deletion.mean(axis=0) if deletion.ndim > 1 else deletion
...
def compare_classification(pred, target):
    L = min(len(pred), len(target))
    pred, target = pred[:L], target[:L]
    matches = (pred == target).sum()
    return matches / L

def compare_regression(pred, target):
    L = min(len(pred), len(target))
    pred, target = pred[:L], target[:L]
    return np.mean((pred - target) ** 2)

def compare_all_features(results, features):
    af2 = process_af2_features(features)

    metrics = {
        "aatype_acc": compare_classification(results["aatype"][0], af2["aatype"]),
        "residue_index_acc": compare_classification(results["residue_index"][0], af2["residue_index"]),
        "msa_acc": compare_classification(results["msa"][0], af2["msa"]),
        "deletion_mse": compare_regression(results["deletion_matrix"][0], af2["deletion_matrix"]),
    }

    print("\n📊 Comparación con AlphaFold2:")
    print(f"  AAtype Accuracy       : {metrics['aatype_acc']:.4f}")
    print(f"  Residue Index Accuracy: {metrics['residue_index_acc']:.4f}")
    print(f"  MSA Accuracy          : {metrics['msa_acc']:.4f}")
    print(f"  Deletion Matrix MSE   : {metrics['deletion_mse']:.4f}")
...
fasta_path = "/home/david/AlphaFoldData/ubiquitna_humana.fasta"
model_path = "final_enhanced_model.pth"
features_path = "/home/david/AlphaFoldData/AlphaFoldOutputs/ubiquitna_humana/features_01.pkl"

# 1️⃣ Predicción del modelo
results = predict_from_fasta(fasta_path, model_path)

# 2️⃣ Cargar features de AlphaFold
features = load_alphafold_features(features_path)

# 3️⃣ Comparar todas las features
metrics = compare_all_features(results, features) 